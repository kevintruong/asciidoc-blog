...
For example: we create new change in the current tree object by modify file README.adoc. Currenly,
the `README.adoc` have hashing is `58731f2bf9d5c803061c62d63012931f1e3e7b68`
...
----
naturally, because the file `README.adoc` is change is content then led to the hashing is change from the old hashing number
to new one is `917449df8f8ad38979c86af887a76aed6ca341f2` the as expected the hashing of tree object is change also. Or in another words,
git create new tree object to store the current snapshot of data.
